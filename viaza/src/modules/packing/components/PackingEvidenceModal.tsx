 ...
 * Modal para tomar foto de evidencia de un ítem de maleta.
 * Usa @capacitor/camera real — sin mocks.
 */
...
export function PackingEvidenceModal({
  open, onClose, itemId, itemLabel, travelerId, travelerName,
}: PackingEvidenceModalProps) {
  const { t } = useTranslation();
  const addPackingEvidence = useAppStore((s) => s.addPackingEvidence);
  const removePackingEvidence = useAppStore((s) => s.removePackingEvidence);
  const existingEvidence = useAppStore((s) =>
    s.packingEvidence.find((e) => e.itemId === itemId && e.travelerId === travelerId)
  );
  ...
  async function handleTakePhoto() {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Error al tomar foto';
      setError(msg);
    } finally {
      setLoading(false);
    }
        ...
        {existingEvidence ? (
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ position: 'relative', borderRadius: 16, overflow: 'hidden' }}>
              <img
                src={existingEvidence.photoDataUrl}
                alt={itemLabel}
              ...
              <div style={{
                ...
                {new Date(existingEvidence.takenAt).toLocaleString()}
              </div>
            </div>
            ...
            >
              {loading ? (
                <motion.div
                  ...
                  style={{ width: 20, height: 20, borderRadius: '50%', border: '2px solid rgba(18,33,46,0.15)', borderTopColor: '#12212E' }}
                />
              ) : (
                <>
